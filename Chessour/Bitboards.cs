...
namespace Chessour
{
    static class Bitboards
    {
        public struct MagicStruct
        {
            ...
            public int GetTableIndex(Bitboard occupancy)
            {
                return offset + (int)((ulong)(occupancy & mask) * magic >> shift);
            }
        }
        ...
        static readonly Bitboard[] bishopAttacks = new Bitboard[0x1480];
        static readonly Bitboard[] rookAttacks = new Bitboard[0x19000];
        ...
        public static Bitboard Attacks(PieceType pt, Square s, Bitboard occupancy)
        {
            return pt switch
            {
                Bishop => bishopAttacks[bishopMagics[(int)s].GetTableIndex(occupancy)],
                Rook => rookAttacks[rookMagics[(int)s].GetTableIndex(occupancy)],
                Queen => bishopAttacks[bishopMagics[(int)s].GetTableIndex(occupancy)] | rookAttacks[rookMagics[(int)s].GetTableIndex(occupancy)],
                _ => Attacks(pt, s),
            };
        }
        ...
        static Bitboards()
        {
            static void CalculateMagics(PieceType pt, MagicStruct[] target, Bitboard[] table)
            {
            ...
            CalculateMagics(Bishop, bishopMagics, bishopAttacks);
            CalculateMagics(Rook, rookMagics, rookAttacks);
